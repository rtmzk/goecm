...
func GetSqliteFactoryOr(opts *genericoptions.SQLiteOptions) (store.Factory, error) {
	...
	once.Do(func() {
		...
		dbIns, err = db.NewSqlite(options)
		initDB(dbIns)
		sqliteFactory = &datastore{
			db: dbIns,
		}
	})
	if sqliteFactory == nil || err != nil {
		return nil, fmt.Errorf("failed to get sqlite store factory, mysqlFactory: %v,error: %w", sqliteFactory, err)
	}
